...
import com.stardict.shared.reflection.ReflectionUtils;
...
public class TermuxUtils {
     ...
     *
     * If your third-party app is targeting sdk `30` (android `11`), then it needs to add `com.stardict`
     * package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in its
     * `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... com.stardict/......} BLOCKED`
     ...
     * <manifest
     *     <queries>
     *         <package android:name="com.stardict" />
     *    </queries>
     * </manifest>
     * }
     *
    ...
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream = context.getResources().openRawResource(com.stardict.shared.R.raw.apt_info_script);
        try {
