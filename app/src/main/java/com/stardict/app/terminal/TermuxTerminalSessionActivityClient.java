...
import com.stardict.shared.interact.ShareUtils;
import com.stardict.shared.termux.shell.command.runner.terminal.TermuxSession;
import com.stardict.shared.termux.interact.TextInputDialogUtils;
...
import com.stardict.shared.termux.settings.properties.TermuxPropertyConstants;
import com.stardict.shared.termux.terminal.io.BellHandler;
import com.stardict.shared.logger.Logger;
import com.stardict.terminal.TerminalColors;
import com.stardict.terminal.TerminalSession;
import com.stardict.terminal.TerminalSessionClient;
import com.stardict.terminal.TextStyle;
...
public class TermuxTerminalSessionActivityClient extends TermuxTerminalSessionClientBase {
    ...
    private synchronized void loadBellSoundPool() {
        if (mBellSoundPool == null) {
            ...
            try {
                mBellSoundId = mBellSoundPool.load(mActivity, com.stardict.shared.R.raw.bell, 1);
            } catch (Exception e){
                // Catch java.lang.RuntimeException: Unable to resume activity {com.stardict/com.stardict.app.TermuxActivity}: android.content.res.Resources$NotFoundException: File res/raw/bell.ogg from drawable resource ID
    ...
    public void showSessionBlockedToast(String reason) {
        if (mActivity == null) return;
        Logger.showToast(mActivity, reason + ". Please wait...", false);
    }
    ...
    public void addNewSession(boolean isFailSafe,
            String sessionName, String[] args) {
        if (mActivity.getTermuxService() == null) {
            Logger.showToast(mActivity, "Terminal service not running", true);
            return;
        }

        TerminalSession currentSession = mActivity.getCurrentSession();
        String workingDirectory =
            currentSession == null ? null : currentSession.getCwd();

        String executable = null;
        if (!isFailSafe) {
            File startupFile = 
                new File(mActivity.getFilesDir(), "startup.sh");
            if (startupFile.exists()) {
                executable = "/system/bin/sh";
                // Pass 'startup.sh' as a parameter to shell
                if (args != null && args.length > 0) {
                    String[] combined = new String[args.length + 1];
                    combined[0] = startupFile.getAbsolutePath();
                    System.arraycopy(args, 0, combined, 1, args.length);
                    args = combined;
                } else {
                    args = new String[]{startupFile.getAbsolutePath()};
                }
            }
        }

        TermuxSession newTermuxSession =
            mActivity.getTermuxService().createTermuxSession(
                executable, sessionName, workingDirectory,
                isFailSafe, args);
        ...
        if (newTermuxSession == null) {
            ...
            int sessionCount = mActivity.getTermuxService().getTermuxSessionsSize();

            if ((isInstalling || isUpdating) && sessionCount > 0) {
                String reason = isInstalling ? 
                    "First install in progress" : "Update in progress";
                Logger.showToast(mActivity, reason + ". Please wait...", true);
            } else if (isInstalling || isUpdating) {
                Logger.showToast(mActivity,
                        "Session creation failed. Check bootstrap and logs.", true);
            } else {
                Logger.showToast(mActivity,
                        "Failed to create session.", true);
            }
            return;
        }
    ...
    public void checkForFontAndColors() {
        checkForFontAndColors(false);
    }

    public void checkForFontAndColors(boolean force) {
        try {
            ...
            // Load font if first call, or if forced (e.g. after bootstrap extraction)
            if (force || !mFontLoaded) {
    ...
    /**
     * Copy bundled font from assets to the Termux font file path.
     * Called during bootstrap extraction to set up the default custom font.
     * Respects user's own font if they placed one at the same path.
     */
    public static void setupBundledFont(Context context) {
        try {
            File fontDir = new File(TermuxConstants.TERMUX_FONT_FILE.getParent());
            if (!fontDir.exists()) {
                fontDir.mkdirs();
            }

            if (TermuxConstants.TERMUX_FONT_FILE.exists()) {
                return; // User font takes priority
            }
            ...
            try (java.io.InputStream is = context.getAssets().open("font.ttf");
                 java.io.FileOutputStream fos = new java.io.FileOutputStream(TermuxConstants.TERMUX_FONT_FILE)) {
            ...
            Logger.logInfo(LOG_TAG, "Bundled font installed to " + TermuxConstants.TERMUX_FONT_FILE_PATH);
        } catch (Exception e) {
            Logger.logWarn(LOG_TAG, "Failed to install bundled font: " + e.getMessage());
        }
    }
